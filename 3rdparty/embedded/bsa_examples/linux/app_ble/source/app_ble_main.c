...
enum
{
    ...
    APP_BLESE_MENU_SEND_NOTIF,

#if defined(APP_BLE2_BRCM_INCLUDED) && (APP_BLE2_BRCM_INCLUDED == TRUE)
    APP_BLE_MENU_LE1_PACKET_EXT,
...
void app_ble_display_menu (void)
{
    ...
    APP_INFO1("\t%d => Enalbe APCF(Advertising Packet Content Filter)",APP_BLE_MENU_APCF_ENABLE);
    ...
    APP_INFO1("\t%d => Cache refresh", APP_BLECL_MENU_REFRESH);
#if defined(APP_BLE_OTA_FW_DL_INCLUDED) && (APP_BLE_OTA_FW_DL_INCLUDED == TRUE)
...
void app_ble_menu(void)
{
    ...
    do
    {
        ...
        case APP_BLE_MENU_CONFIG_BLE_CONN_PARAM:
            conn_param.preference = app_get_choice("Is this preferred value?(0 = no, 1 = yes)");
        ...
        case APP_BLE_MENU_APCF_ENABLE:
            app_ble_apcf_enable();
            break;

        case APP_BLE_MENU_APCF_DISABLE:
            app_ble_apcf_disable();
            break;

        case APP_BLE_MENU_APCF_CONFIG:
            app_ble_apcf_cfg();
            break;
        ...
        case APP_BLECL_MENU_PREPARE_WRITE:
            app_ble_client_prepare_write();
            break;

        case APP_BLECL_MENU_EXECUTE_WRITE:
            app_ble_client_execute_write();
            break;
        ...
        case APP_BLESE_MENU_SEND_NOTIF:
            app_ble_server_send_notification();
            break;

#if defined(APP_BLE2_BRCM_INCLUDED) && (APP_BLE2_BRCM_INCLUDED == TRUE)
        case APP_BLE_MENU_LE1_PACKET_EXT:
